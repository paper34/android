	...
	public static void main(String[] args) {
		...
		login.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// 로그인 버튼 클릭 시 dao를 통해 입력값이 db에 있는지 확인한다.
				try {
					boolean res = dao.login(user_id, user_pw); // 입력값 id, pw를 db에서 확인
					if (res) {
						JOptionPane.showMessageDialog(f, "로그인성공");

						ProHome t1 = new ProHome();
						t1.main(null); // 로그인이 완료되면 Login화면을 꺼지고 다음 Main화면으로 이동
						f.setVisible(false);
					} else {
			...
			public void actionPerformed(ActionEvent e) {
				...
				read.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						// id 파라메터 값을 dao를 통해 db에서 select를 하여
						// 유저에게 다른 컬럼값들을 보여준다
						try {
				...
				read_1.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						// 탈퇴버튼을 누르면 dao에서 id 파라메터를 받아 해당 db row를 
						// delete를 하여 탈퇴처리를 한다.
						try {
							dao.delete(bag);
							// 탈퇴하기전에 유저에게 최종점을 confirm을 받는다.
