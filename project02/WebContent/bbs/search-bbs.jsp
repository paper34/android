		...
		<div class="column middle">
			...
			<table class="table table-hover text-center" id="click">
				...
				<tbody style="font-size: 15px">
					...
					<%
						BbsDAO d = new BbsDAO();
						BbsVO vo = new BbsVO();
						ArrayList<BbsVO> list = d.search(pageNumber, title);
						for (int i = 1; i < list.size(); i++) {
					%>

					<tr class='clickable-row' data-href='./read-bbs.jsp?no=<%=list.get(i).getNo()%>'>
					
						<!-- 글번호 -->
						<th>
							<input type="hidden" name="no" id="no" class="no"
									value="<%=list.get(i).getNo()%>"><%=list.get(i).getNo()%>
						</th>
						...
						<th>
							<input type="hidden" name="count" id="count" class="count"
								 	value="<%=list.get(i).getCount()%>"><%=list.get(i).getCount()%>
						</th>
					</tr>
			...
			<div class="text-center">
				...
				<ul class="pagination justify-content-center" style="margin: 20px 0">
					<li class="page-item">
						<a class="page-link" href="main-bbs.jsp?pageNumber=<%=pageNumber - 1%>">이전</a>
					</li>
				...
				%>
					...
					<li class="page-item">
						<a class="page-link"href="main-bbs.jsp?pageNumber=<%=pageNumber + 1%>">다음</a>
					</li>
				</ul>
				<%
					}
			...
			<div class="input-group mb-3">
				<form class="form-inline" action="search-bbs.jsp">
					...
					<button class="btn btn-dark	" type="submit" onclick="check()">검색</button>
				</form>
			</div>
		</div>
